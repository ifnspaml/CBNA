...
class ArgumentsBase(object):
    ...
    def _harness_init_model(self):
        self.ap.add_argument(
            '--model-type', type=str, default='vgg', choices=('resnet', 'vgg'),
        ...
        self.ap.add_argument(
            '--model-cbna-bn-inference', default=False, action='store_true',
            help='Do not load the training state but only the model weights'
        )
    ...
    def _training_init_adaptation(self):
        self.ap.add_argument(
            '--adaptation-loaders', type=str, default='kitti_kitti_train',
            help='Comma separated list of adaptation dataset loaders from loaders/adaptation/train.py to use '
                 'for training'
        )

        self.ap.add_argument(
            '--adaptation-window', type=str, default="0",
            help='specify the window used in video adaptation'
        )

        self.ap.add_argument(
            '--adaptation-batch-size', type=int, default=DEFAULT_ADAPTATION_BATCH_SIZE,
        ...
        self.ap.add_argument(
            '--adaptation-bn-momentum', type=float, default=0.2,
            help='Momentum for the BatchNorm layer of the shared encoder'
        )

        self.ap.add_argument(
            '--adaptation-batches', type=int, default=500,
            help='Number of elements in the adaptation loader'
        )
...
class AdaptationArguments(ArgumentsBase):
    DESCRIPTION = 'Adaptation arguments'
