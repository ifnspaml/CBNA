...
def kitti_train(resize_height, resize_width, crop_height, crop_width, batch_size, num_workers):
    """A loader that loads images and ground truth for segmentation from the
    gta5 training set.
    """
    ...
    transforms = [
        ...
        tf.ToTensor(),
        tf.NormalizeZeroMean(),
        tf.AddKeyValue('domain', 'kitti_train_seg'),
        tf.AddKeyValue('purposes', ('segmentation', 'domain')),
        tf.AddKeyValue('num_classes', num_classes)
    ]
    ...
    print(f"  - Can use {len(dataset)} images from kitti for segmentation training", flush=True)
